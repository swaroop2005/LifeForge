...
const ROLE_META = {
  patient: { label: 'Patient', Icon: Heart, desc: 'Track your blood disorder journey', iconCls: 'bg-red-100 text-red-600' },
  donor:   { label: 'Donor',   Icon: Droplets, desc: 'Record donations and see your impact', iconCls: 'bg-orange-100 text-orange-600' },
  hospital:{ label: 'Hospital',Icon: Building2, desc: 'Request blood and trace donations', iconCls: 'bg-blue-100 text-blue-600' },
}
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-3xl shadow-2xl p-8">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-gray-900">{meta.label} Portal</h1>
            ...
            {['login', 'register'].map(m => (
              <button key={m} onClick={() => { setMode(m); setError('') }}
                className={`flex-1 py-2.5 text-sm font-semibold rounded-xl capitalize transition-all
                  ${mode === m ? 'bg-white text-red-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}>
              ...
              <>
                ...
                {role === 'patient' && (
                  <>
                    ...
                    <div>
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1">Next Transfusion Date</label>
                      <input type="date" value={form.next_transfusion_date}
                        onChange={e => set('next_transfusion_date', e.target.value)}
                        className={inputCls} />
                      <p className="text-xs text-gray-400 mt-1">Leave blank if unknown — you can update later</p>
                    </div>
                  </>
                )}
                ...
                {role === 'donor' && (
                  <>
                    <div>
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1">Last Donated On (if ever)</label>
                      <input type="date" value={form.last_donation_date}
                        onChange={e => set('last_donation_date', e.target.value)}
                        className={inputCls} />
                      <p className="text-xs text-gray-400 mt-1">We'll calculate when you're eligible to donate again</p>
                    </div>
                    <label className="flex items-center gap-3 text-sm text-gray-600 cursor-pointer bg-gray-50 px-4 py-3 rounded-xl">
