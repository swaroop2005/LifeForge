...
export default function HospitalDashboard() {
  ...
  return (
    <Layout role="hospital" activeTab={tab} setTab={t => { setTab(t); setMsg('') }}>
      ...
      {tab === 'requests' && (
        <div>
          ...
          <div className="space-y-3">
            {requests.map(r => (
              <div key={r.id} className="bg-white rounded-2xl border border-gray-100 p-5 flex justify-between items-center shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 capitalize">{r.urgency} · {r.notes}</p>
                  </div>
                </div>
      ...
      {tab === 'blood-banks' && (
        <div>
          <h2 className="text-lg font-bold text-gray-900 mb-5">Nearby Blood Banks</h2>
          <div className="space-y-3">
            {nearbyBanks.map(b => (
                ...
                <div className="p-5 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">{b.city}, {b.state}
                  ...
                  <button onClick={() => toggleStock(b.id)}
                    className={`text-xs px-4 py-2 rounded-xl font-semibold transition-colors
                      ${bankStock[b.id] ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-red-50 hover:text-red-600'}`}>
                  ...
                  <div className="px-5 pb-5 pt-0 border-t border-gray-50">
                    <div className="grid grid-cols-4 gap-3 mt-4">
                      {bankStock[b.id].map(s => (
                        <div key={s.blood_type} className={`rounded-xl p-3 text-center border ${s.units < 10 ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-100'}`}>
                          <BloodTypeTag type={s.blood_type} />
                          <p className={`text-sm font-bold mt-2 ${s.units < 10 ? 'text-red-600' : 'text-gray-700'}`}>{s.units}</p>
      ...
      {tab === 'trace-blood' && (
        <div className="max-w-xl">
          <h2 className="text-lg font-bold text-gray-900 mb-2">Trace Blood to Patient</h2>
          <p className="text-sm text-gray-400 mb-6">Link a donation record to a patient request. This creates a blood journey and notifies the patient.</p>
          <div className="bg-white rounded-2xl border border-gray-100 p-7 shadow-sm">
            <form onSubmit={traceBlood} className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-2">Donation ID</label>
              ...
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-2">Patient Request ID</label>
              ...
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-2">Message to Patient</label>
