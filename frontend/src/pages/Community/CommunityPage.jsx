...
const ROLE_STYLES = {
  patient:  { cls: 'bg-rose-500/15 text-rose-400',   label: 'Patient' },
  donor:    { cls: 'bg-red-500/15 text-red-400',      label: 'Donor' },
  hospital: { cls: 'bg-blue-500/15 text-blue-400',    label: 'Hospital' },
  admin:    { cls: 'bg-purple-500/15 text-purple-400', label: 'Admin' },
}

const DISEASE_COLORS = {
  thalassemia:    'bg-red-500/10 text-red-400 border-red-500/30',
  sickle_cell:    'bg-orange-500/10 text-orange-400 border-orange-500/30',
  hemophilia:     'bg-purple-500/10 text-purple-600 border-purple-200',
  anemia:         'bg-yellow-500/10 text-yellow-400 border-yellow-500/30',
  aplastic_anemia:'bg-blue-500/10 text-blue-600 border-blue-500/30',
  donor:          'bg-green-500/10 text-green-400 border-green-500/30',
  hospital:       'bg-sky-50 text-sky-600 border-sky-200',
  admin:          'bg-ink text-zinc-400 border-white/10',
}
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="md:col-span-2 space-y-5">
              ...
              {token ? (
                <div className="bg-panel rounded-2xl border border-white/10 shadow-sm overflow-hidden">
                  {!showForm ? (
                    <button onClick={() => setShowForm(true)}
                      ...
                      <span className="text-sm text-zinc-500">Share something with the community...</span>
                    </button>
                  ) : (
                    <form onSubmit={submit} className="p-5">
                      ...
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <div>
                          <p className="text-sm font-bold text-zinc-100">{userName}</p>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className={`text-xs font-semibold px-2 py-0.5 rounded-lg capitalize ${ROLE_STYLES[userRole]?.cls || 'bg-white/10 text-zinc-400'}`}>
                      ...
                      <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                        ...
                        className="w-full bg-ink border border-white/10 rounded-xl px-4 py-3 text-sm font-semibold text-zinc-200 outline-none focus:border-red-400 mb-3" />
                      <textarea value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))}
                        placeholder="What's on your mind? Share your experience, ask for help, or encourage others..." required
                        rows={4} className="w-full bg-ink border border-white/10 rounded-xl px-4 py-3 text-sm text-zinc-300 outline-none focus:border-red-400 resize-none mb-3" />
                      <select value={form.disease_tag} onChange={e => setForm(f => ({ ...f, disease_tag: e.target.value }))}
                        className="w-full bg-ink border border-white/10 rounded-xl px-4 py-2.5 text-sm text-zinc-400 outline-none focus:border-red-400 mb-4">
                      ...
                      <div className="flex justify-between items-center">
                        <button type="button" onClick={() => setShowForm(false)} className="text-sm text-zinc-500 hover:text-zinc-400">
                          Cancel
                        </button>
                ...
                <div className="bg-panel rounded-2xl border border-white/10 shadow-sm p-5 flex items-center justify-between">
                  <p className="text-sm text-zinc-400">Sign in to share your story or ask the community.</p>
                  <button onClick={() => nav('/')}
                ...
                {FILTERS.map(f => (
                  <button key={f.key} onClick={() => setFilter(f.key)}
                    className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold transition-all
                      ${filter === f.key ? 'bg-red-600 text-white' : 'bg-panel border border-white/10 text-zinc-400 hover:border-red-500/50 hover:text-red-500'}`}>
                    {f.label}
                  </button>
                ))}
              </div>
              ...
              {loading && (
                <div className="space-y-3">
                  {[1, 2, 3].map(i => (
                    <div key={i} className="bg-panel rounded-2xl border border-white/10 p-5 animate-pulse">
              ...
              {!loading && filtered.length === 0 && (
                <div className="bg-panel rounded-2xl border border-dashed border-white/10 p-12 text-center">
                  ...
                  <p className="text-zinc-500 text-sm">No posts yet in this category. Be the first to share!</p>
                </div>
              )}

              {!loading && filtered.map(p => (
                <article key={p.id} className="bg-panel rounded-2xl border border-white/10 shadow-sm hover:shadow-md transition-shadow p-5">
                  {/* Author row */}
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-red-500/10 rounded-xl flex items-center justify-center text-red-400 font-bold text-sm">
                        {p.author_name?.charAt(0) || '?'}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-zinc-100">{p.author_name || 'Anonymous'}</p>
                        <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                          {/* Role tag */}
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-lg capitalize ${ROLE_STYLES[p.author_type]?.cls || 'bg-white/10 text-zinc-400'}`}>
                            {ROLE_STYLES[p.author_type]?.label || p.author_type}
                          </span>
                          {/* Blood type */}
                          {p.author_blood_type && (
                            <span className={`text-xs font-bold px-2 py-0.5 rounded-lg ${BLOOD_COLORS[p.author_blood_type] || 'bg-white/10 text-zinc-400'}`}>
                              {p.author_blood_type}
                            </span>
                          )}
                          {/* Disease tag */}
                          {p.disease_tag && p.disease_tag !== p.author_type && (
                            <span className={`text-xs font-semibold px-2 py-0.5 rounded-lg border capitalize ${DISEASE_COLORS[p.disease_tag] || 'bg-ink text-zinc-400 border-white/10'}`}>
                    ...
                    <p className="text-xs text-zinc-600 whitespace-nowrap ml-2">
                  ...
                  <p className="text-sm text-zinc-400 leading-relaxed">{p.content}</p>
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-50 flex items-center gap-4">
                    <button onClick={() => !p.liked_by_me && like(p.id)}
                      ...
                      className={`flex items-center gap-1.5 text-xs transition-colors group ${p.liked_by_me ? 'text-red-500 cursor-default' : 'text-zinc-500 hover:text-red-500'}`}>
            ...
            <div className="space-y-5">
              {/* About */}
              <div className="bg-panel rounded-2xl border border-white/10 shadow-sm p-5">
                <h3 className="font-bold text-zinc-100 mb-3">About Community</h3>
              ...
              {/* Tags */}
              <div className="bg-panel rounded-2xl border border-white/10 shadow-sm p-5">
                <h3 className="font-bold text-zinc-100 mb-3">Browse by Role</h3>
                <div className="flex flex-wrap gap-2">
                  {[
                    { key: 'all', label: 'All Posts', cls: 'bg-white/10 text-zinc-300 border-white/10' },
                    { key: 'patient', label: 'Patients', cls: 'bg-red-500/10 text-red-400 border-red-500/30' },
                    { key: 'donor', label: 'Donors', cls: 'bg-orange-500/10 text-orange-400 border-orange-500/30' },
                    { key: 'hospital', label: 'Hospitals', cls: 'bg-blue-500/10 text-blue-400 border-blue-500/30' },
                  ].map(({ key, label, cls }) => (
